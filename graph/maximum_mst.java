...
class Graph
{
  ...
  void addEdge(int v,int w,int wt)
  {
    ...
    void top_down_max_heapify(ArrayList<Anode>arr,int i)
	{ 
		int n=arr.size();
		while(2*i+1<n)
		{
			...
			if(arr.get(i).priority < arr.get(max).priority)
			 {
			   //int temp=arr.get(i).;
    ...
    void build_max_heap(ArrayList<Anode>arr)
	{
		//System.out.println(arr.size()/2);
		for(int i=arr.size()/2-1;i>=0;i--)
		{
			//System.out.println("helklo");
			top_down_max_heapify(arr,i);
		}
		
	}
	void bottom_up_max_heapify(ArrayList<Anode>arr,int i)
	{
		while((i-1)/2 >=0)
		{
			int parent=(i-1)/2;
    ...
    void add_max_heap(ArrayList<Anode>arr,int val,int []dist)
    {
        ...
        bottom_up_max_heapify(arr,arr.size()-1);
    ...
    */
    int delete_max(ArrayList<Anode>arr)
    {
    ...
    //	System.out.println("after updated min heap= "+arr);
    	if(prev<val)
    	{
            bottom_up_max_heapify(arr,index);
    	    	
    	}
    	else
    	{
    		top_down_max_heapify(arr,index);
    	}
    }	
    ...
    void shortest(int s)
    {
        ...
        build_max_heap(arr);
        dist[s]=0;
        add_max_heap(arr,s,dist);
        visited[s]=true;
        
        
        while(!arr.isEmpty())
        {
          int u=delete_max(arr);  
