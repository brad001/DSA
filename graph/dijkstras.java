  ...
  void addEdge(int v,int w,int wt)
  {
	...
	void bottom_up_min_heapify(ArrayList<Anode>arr,int i)
	{
		...
		//swap if inserted is less than parent
			...
			{
				int temp=arr.get(i).priority;
                arr.get(i).priority=arr.get(parent).priority;
                arr.get(parent).priority=temp;
        ...
        while(!arr.isEmpty())
        {
          ...
          for(int i=0;i<adj[u].size();i++)
          {
            ...
            if(!visited[v])    
            {
              ...
              dist[v]=dist[u]+adj[u].get(i).weight;
            ...
            {
               if(adj[u].get(i).weight + dist[u] < dist[v])
               {
                   System.out.println(v+" is visted");
                 int new_priority=adj[u].get(i).weight+dist[u];  
                 dist[v]=new_priority;  
